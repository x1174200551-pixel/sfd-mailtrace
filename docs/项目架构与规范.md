...
│
├── backend/                       # Java 后端（包名 com.ntn.fziot.mailtrace）
│   ├── pom.xml
│   └── src/
│       ├── main/
│       │   ├── java/
...
| 样式 | **Ant Design 组件为主**，antd 不覆盖的场景用手写 CSS 或内联 style |
| 组件选型 | **所有新页面优先使用 antd 组件（Table、Form、Card、Drawer、DatePicker 等）**，保持风格统一；antd 组件不满足时再手写，但需与 antd 视觉对齐 |
...
| v0.5 | 2026-07-23 | 版本调整：JDK 21→17、SB 3.5→3.2、springdoc 2.8→2.5；包名 com.sfonda→com.ntn.fziot |
| v0.6 | 2026-07-23 | 前端 UI 组件库切换：移除 shadcn/ui + Tailwind，改用 Ant Design 6.x；新增组件选型规范 |
