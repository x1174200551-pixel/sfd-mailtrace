...
public class JakartaMailImapFetchClient implements ImapFetchClient {
    ...
    @Override
    public List<ParsedMail> fetchUnseenMessages(ImapFetchConfig config) throws Exception {
        long start = System.currentTimeMillis();
        log.info("开始 IMAP 拉取（含解析） host={} port={} ssl={} username={} folder={}",
                config.host(), config.port(), config.sslEnabled(), config.username(), config.folder());
        Store store = null;
        Folder folder = null;
        ...
        try {
            ...
            if (unseen != null) {
                for (Message msg : unseen) {
                    try {
                        ParsedMail parsed = IncomingMailParser.parse(msg);
                        result.add(parsed);
                        // 标记已读，避免重复拉取
                        msg.setFlag(Flags.Flag.SEEN, true);
                    } catch (Exception exception) {
                        log.warn("单封邮件解析失败 subject={} error={}",
                                msg.getSubject(), exception.getMessage());
                        // 单封失败不影响其他邮件
                    }
                }
            }
            ...
            long elapsed = System.currentTimeMillis() - start;
            log.info("IMAP 拉取（含解析）完成 host={} folder={} username={} 邮件数={} 总耗时={}ms",
                    config.host(), config.folder(), config.username(), result.size(), elapsed);
            return result;
        } finally {
            if (folder != null && folder.isOpen()) {
                try {
